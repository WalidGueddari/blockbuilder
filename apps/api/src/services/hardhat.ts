...
export class HardhatService {
  ...
  async generateHardhatDockerCompose(networkId: string): Promise<void> {
    const { networkBinDir, hardhatDockerImage, hardhatTemplateFile } = config;
    try {
      ...
      const command = `NET_ID=${networkId} HARDHAT_TEMPLATE_FILE=${hardhatTemplateFile} PRIVATE_KEY=${alloc.private_key}   HARDHAT_DOCKE_IMAGE=${hardhatDockerImage} ${networkBinDir}/generate_docker_compose_hardhat.sh`;
