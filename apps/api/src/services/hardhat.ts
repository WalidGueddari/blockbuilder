...
export class HardhatService {
  ...
  async generateHardhatDockerCompose(networkId: string, network_dns: string): Promise<void> {
    ...
    try {
      ...
      const command = `NET_ID=${networkId} HARDHAT_TEMPLATE_FILE=${hardhatTemplateFile} PRIVATE_KEY=${alloc.private_key} DNS_PLACEHOLDER=${network_dns} HARDHAT_DOCKE_IMAGE=${hardhatDockerImage} ${networkBinDir}/generate_docker_compose_hardhat.sh`;
